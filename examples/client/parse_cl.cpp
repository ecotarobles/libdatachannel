...
Cmdline::Cmdline (int argc, char *argv[]) // ISO C++17 not allowed: throw (std::string )
{
  ...
  static struct option long_options[] =
  {
    ...
    {"peerSession", no_argument, NULL, 'p'},
    {"webrtcMustUseWebSocketServerIpAddress", no_argument, NULL, 'q'},
  ...
  optind = 0;
  while ((c = getopt_long (argc, argv, "s:t:w:x:enpqh", long_options, &optind)) != - 1)
    {
      switch (c)
        {
        case 'e':
...
libdatachannel client implementing WebRTC Data Channels with WebSocket signaling\n\
   ...
   [ -p ] [ --peerSession ] (type=FLAG)\n\
          Session token is a peer ID rather than a true token.\n\
   [ -q ] [ --webrtcMustUseWebSocketServerIpAddress ] (type=FLAG)\n\
          EXPERIMENTAL: only forward local candidates with IP address of web socket server.\n\
